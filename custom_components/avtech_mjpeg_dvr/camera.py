"""Generátor testovacího pruhovaného obrazu pro ověření chodu integrace."""
import logging
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    async_add_entities([AvtechStripedCamera()])
    return True

class AvtechStripedCamera(Camera):
    """Kamera generující dynamický pruhovaný vzor."""

    def __init__(self):
        """Inicializace."""
        super().__init__()
        self._attr_name = "AV-TECH Testovací Pruhy"
    ...
    async def async_camera_image(self, width=None, height=None):
        """Vrací platný binární JPEG obrázek s černobílými pruhy."""
        return b"".join([
